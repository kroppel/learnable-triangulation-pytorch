...
'''
    Generate `labels.npy` for multiview `cmu-multiperson.py`
    Usage: `python3 generate-labels-npy.py <path/to/cmu-multiperson-data-root> <path/to/cmu-multiperson-bbox-detections>`
'''
...
cmu-multiperson (default):
$ python3 generate-labels-npy.py $THIS_REPOSITORY/data/cmu $THIS_REPOSITORY/data/cmu/cmu-multiperson-bboxes.npy 4
"""
...
try:
    cmu_multiperson_root = sys.argv[1]
...
destination_file_path = os.path.join(
    cmu_multiperson_root, f'cmu-multiperson-multiview-labels-{BBOXES_SOURCE}bboxes.npy')

assert os.path.isdir(cmu_multiperson_root), "Invalid data directory '%s'\n%s" % (cmu_multiperson_root, USAGE_PROMPT)
...
print(f"\nFinding actions, frames and cameras in {cmu_multiperson_root}")
...
for action_name in os.listdir(cmu_multiperson_root):
    # Make sure that this is actually a scene
    # and not sth like 'scripts' or 'matlab'
    if '_ultimatum' not in action_name:
        continue

    data = {}

    action_dir = os.path.join(cmu_multiperson_root, action_name)
    ...
    # Also simultaneously extract person data out of BBOX data
    data['camera_data'] = {}
    data['max_persons'] = 0
    for camera_name in data['camera_names']:
        data['camera_data'][camera_name] = camera_data[camera_name]

        for frame_name in data['valid_frames']:
            try:
                n_persons = len(bbox_data[action_name][camera_name][int(frame_name)])
                data['max_persons']  = max(data['max_persons'], n_persons)
...
def load_table_segment(data, action_idx, action_name):
    ...
    n_frames = len(data['valid_frames'])
    n_people = data['max_people']
    table_segment = np.empty(n_frames * n_people), dtype=table_dtype)
    # Idea: 
    #   frame n: [ [person 0, person 1, ... , person n] ]

    for frame_idx, frame_name in enumerate(data['valid_frames']):
        # TODO: Need to create a separate cmu panoptic parser (or modify existing one to work with multipersons)
        for person_id in n_people:
            ...
            idx = n_frames * frame_idx + person_id

            person_ids.add(person_id)
            table_segment[idx]['person_id'] = person_data['id']
            table_segment[idx]['action_idx'] = action_idx
            table_segment[idx]['frame_name'] = int(frame_name)
            ...
            for camera_idx, camera_name in enumerate(retval['camera_names']):
                try:
                    table_segment[idx]['bbox_by_camera_tlbr'][camera_idx] = bbox_data[action_name][camera_name][int(frame_name)][person_id]
                except KeyError:
                    print(
                        f"Missing bbox data {action_name}, {camera_name}, {person_id}.. Ignoring")
                    missing_data.append((action_name, camera_name))
                    table_segment[idx]['bbox_by_camera_tlbr'][camera_idx] = [0, 0, 0, 0, 0]
...
for action_idx, action_name in enumerate(retval['action_names']):
    ...
    # Load table segment (all valid frames)
    if USE_MULTIPROCESSING:
...
print('Total frames in CMU multiperson Dataset:', len(retval['table']))
